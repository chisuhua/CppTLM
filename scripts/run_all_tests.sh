...
cd "$ROOT_DIR"
FAILED_PY=0
$PYTEST cpptlm_config/tests/ test/python/test_port_types.py -v --tb=short || ((FAILED_PY++))
echo ""
...
if [[ -f "${BUILD_DIR}/bin/cpptlm_sim" ]]; then
    FAILED_E2E=0
    for cfg in "${ROOT_DIR}/configs/"*_tlm.json "${ROOT_DIR}/configs/tlm_e2e_test.json" "${ROOT_DIR}/configs/cpu_tlm_test.json" "${ROOT_DIR}/configs/crossbar_test.json" "${ROOT_DIR}/configs/cache_chstream_test.json" "${ROOT_DIR}/configs/arbiter_tlm_test.json" "${ROOT_DIR}/configs/traffic_gen_tlm_test.json"; do
        if [[ ! -f "$cfg" ]]; then
            continue
        fi
