...
TEST_CASE("PcieEndpoint: BAR0 doorbell side_effect (250-700ns strong-order)", "[pcie][endpoint][bar][doorbell]") {
    EventQueue eq;
    PcieEndpointTLM ep("pcie_ep", &eq);
    ep.init();

    // 配置门铃寄存器 0x0014, stream_id=0
    REQUIRE(ep.bar_router().add_register(
        0x0014, "GPU_REG_DOORBELL",
        PcieBarRouter::Access::WO,
...
TEST_CASE("PcieEndpoint: BAR0 data-driven registers (no if-else)", "[pcie][endpoint][bar][data-driven]") {
    EventQueue eq;
    PcieEndpointTLM ep("pcie_ep", &eq);
    ep.init();

    // 配置 3 个不同 offset 的寄存器
    REQUIRE(ep.bar_router().add_register(0x0000, "REG0", PcieBarRouter::Access::RW, PcieBarRouter::SideEffect::NONE) == true);
    REQUIRE(ep.bar_router().add_register(0x1000, "REG1", PcieBarRouter::Access::RW, PcieBarRouter::SideEffect::NONE) == true);
    REQUIRE(ep.bar_router().add_register(0x2000, "REG2", PcieBarRouter::Access::RW, PcieBarRouter::SideEffect::NONE) == true);
    ...
    // RO 寄存器写拒绝
    REQUIRE(ep.bar_router().add_register(0x3000, "REG_RO", PcieBarRouter::Access::RO, PcieBarRouter::SideEffect::NONE) == true);
...
TEST_CASE("PcieEndpoint: BAR1 MEM size > 8 descriptor-only TLP (backdoor)", "[pcie][endpoint][bar][mem][backdoor]") {
