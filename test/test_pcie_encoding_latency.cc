...
TEST_CASE("PcieEncodingLatencyModel: Gen5 x1 lane 128B block == 32ns",
    ...
    const uint64_t lat = PcieEncodingLatencyModel::block_latency_ns(
        PcieEncodingLatencyModel::Rate::GEN5, 128, 1);
    REQUIRE(lat == 32u);
}
...
TEST_CASE("PcieEncodingLatencyModel: Gen5 x16 lane 128B block == 2ns",
    ...
    const uint64_t lat = PcieEncodingLatencyModel::block_latency_ns(
        PcieEncodingLatencyModel::Rate::GEN5, 128, 16);
    REQUIRE(lat == 2u);
}
...
TEST_CASE("PcieEncodingLatencyModel: Gen3 x1 lane 128B block == 128ns",
    ...
    const uint64_t lat = PcieEncodingLatencyModel::block_latency_ns(
        PcieEncodingLatencyModel::Rate::GEN3, 128, 1);
    REQUIRE(lat == 128u);
}
...
TEST_CASE("PcieEncodingLatencyModel: Gen4 x4 lane 128B block == 16ns",
    ...
    const uint64_t lat = PcieEncodingLatencyModel::block_latency_ns(
        PcieEncodingLatencyModel::Rate::GEN4, 128, 4);
    REQUIRE(lat == 16u);
}
...
TEST_CASE("PcieEncodingLatencyModel: Gen2 x1 lane 128B block == 204ns",
    ...
    const uint64_t lat = PcieEncodingLatencyModel::block_latency_ns(
        PcieEncodingLatencyModel::Rate::GEN2, 128, 1);
...
TEST_CASE("PcieEncodingLatencyModel: 速率切换 GEN3→GEN5 >= 1µs(含均衡协商)",
    ...
    const uint64_t delay_us = PcieEncodingLatencyModel::rate_switch_delay_us(
        PcieEncodingLatencyModel::Rate::GEN3,
...
TEST_CASE("PcieEncodingLatencyModel: 速率切换 GEN5→GEN1 仍 >= 1µs(反向协商)",
    ...
    const uint64_t delay_us = PcieEncodingLatencyModel::rate_switch_delay_us(
        PcieEncodingLatencyModel::Rate::GEN5,
        PcieEncodingLatencyModel::Rate::GEN1);
    REQUIRE(delay_us >= 1u);
}
...
TEST_CASE("PcieEncodingLatencyModel: 边界 lanes=0 返回 0ns(保护)",
    ...
    const uint64_t lat = PcieEncodingLatencyModel::block_latency_ns(
        PcieEncodingLatencyModel::Rate::GEN5, 128, 0);
    REQUIRE(lat == 0u);
}
...
TEST_CASE("PcieEncodingLatencyModel: 边界 block_size=0 返回 0ns",
          "[pcie][encoding][latency][edge]") {
    const uint64_t lat = PcieEncodingLatencyModel::block_latency_ns(
        PcieEncodingLatencyModel::Rate::GEN5, 0, 1);
    REQUIRE(lat == 0u);
}

TEST_CASE("PcieEncodingLatencyModel: Rate 枚举值映射正确",
...
TEST_CASE("PcieEncodingLatencyModel: rate_gtps() 公开枚举对应数值",
          "[pcie][encoding][latency][enum]") {
    REQUIRE(PcieEncodingLatencyModel::rate_gtps(
        PcieEncodingLatencyModel::Rate::GEN5) == 32u);
    REQUIRE(PcieEncodingLatencyModel::rate_gtps(
        PcieEncodingLatencyModel::Rate::GEN3) == 8u);
    REQUIRE(PcieEncodingLatencyModel::rate_gtps(
        PcieEncodingLatencyModel::Rate::GEN1) == 2u);
}
