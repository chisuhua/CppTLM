...
TEST_CASE("PcieMsixTablePerVf: per-slot tables are distinct",
          "[pcie][sriov][per-vf-msix]") {
    PcieMsixTablePerVf pool;
    REQUIRE(&pool.table_of(0) != &pool.table_of(1));
    REQUIRE(&pool.table_of(1) != &pool.table_of(2));
}

TEST_CASE("PcieMsixTablePerVf: independent vector counts per VF",
          "[pcie][sriov][per-vf-msix]") {
    PcieMsixTablePerVf pool;
...
TEST_CASE("PcieMsixTablePerVf: PF pending does not affect VF0",
          "[pcie][sriov][per-vf-msix]") {
    PcieMsixTablePerVf pool;
...
TEST_CASE("PcieMsixTablePerVf: clear_pending per-VF only",
          "[pcie][sriov][per-vf-msix]") {
    PcieMsixTablePerVf pool;
    pool.configure_vectors(0, 16);
    pool.configure_vectors(1, 16);
...
TEST_CASE("PcieMsixTablePerVf: out-of-range vf_id throws",
          "[pcie][sriov][per-vf-msix]") {
    PcieMsixTablePerVf pool;
