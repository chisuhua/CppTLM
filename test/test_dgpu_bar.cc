...
TEST_CASE("DGpuBar: constructor initializes BAR0 and VRAM", "[dgpu-bar]") {
    tlm::gpu::DGpuBar bar;

    REQUIRE(bar.read_reg(0x00) == 0x10DE); // VENDOR_ID (DWORD at byte 0)
    REQUIRE(bar.vram_base() != nullptr);
    REQUIRE(bar.vram_size() == 256 * 1024 * 1024);
}

TEST_CASE("DGpuBar: repeated init is safe (leak guard)", "[dgpu-bar]") {
    tlm::gpu::DGpuBar bar;
    ...
    REQUIRE(bar.vram_base() != nullptr);
    REQUIRE(bar.vram_size() == 256 * 1024 * 1024);
    REQUIRE(bar.read_reg(0x00) == 0x10DE);
    REQUIRE(bar.read_reg(0x04) == 0x1234);
    REQUIRE(bar.read_reg(0x08) == 0x01);
}
