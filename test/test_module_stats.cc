...
namespace {
    ...
    TEST_CASE("ModuleFactory: enable_metrics creates stats root",
              "[phase8-module-stats][factory]") {
        EventQueue eq;
        ModuleFactory factory(&eq);
    ...
    TEST_CASE("ModuleFactory: enable_metrics(false) does not create root",
              "[phase8-module-stats][factory]") {
        EventQueue eq;
        ModuleFactory factory(&eq);
    ...
    TEST_CASE("ModuleFactory: dump_metrics to file", "[phase8-module-stats][factory]") {
        ...
        REQUIRE(content.find("---------- Begin Simulation Statistics ----------") !=
                std::string::npos);
        REQUIRE(content.find("system.cache.requests") != std::string::npos);
        REQUIRE(content.find("---------- End Simulation Statistics ----------") !=
                std::string::npos);
    ...
    TEST_CASE("ModuleFactory: dump_metrics when disabled does nothing",
              "[phase8-module-stats][factory]") {
        EventQueue eq;
        ModuleFactory factory(&eq);
        // 未启用 metrics
