// test/test_router_vc_arbiter.cc
...
static void inject_flit(RouterTLM* router, unsigned port, uint64_t tid, uint32_t dst,
                        uint8_t vc = 0, uint8_t flit_type = NoCFlitBundle::FLIT_HEAD_TAIL) {
    NoCFlitBundle flit;
...
TEST_CASE("RouterTLM default construction", "[router][vc][tlm]") {
    EventQueue eq;
...
TEST_CASE("RouterTLM XY routing to east node", "[router][vc][tlm]") {
    EventQueue eq;
    RouterTLM router("r0", &eq, 0, 0, 2, 2);
...
TEST_CASE("RouterTLM XY routing to north node", "[router][vc][tlm]") {
    EventQueue eq;
    RouterTLM router("r0", &eq, 0, 0, 2, 2);
...
TEST_CASE("RouterTLM XY routing to west node (wrap)", "[router][vc][tlm]") {
    EventQueue eq;
...
TEST_CASE("RouterTLM local routing", "[router][vc][tlm]") {
    EventQueue eq;
    RouterTLM router("r0", &eq, 0, 0, 2, 2);
...
TEST_CASE("RouterTLM XY routing to south node", "[router][vc][tlm]") {
    EventQueue eq;
    RouterTLM router("r0", &eq, 0, 1, 2, 2);
    uint32_t dst_node = RoutingAlgorithm::coordToNode(0, 0, 2);
    unsigned out_port = router.compute_xy_route(dst_node);
    REQUIRE(out_port == 2);
}
...
TEST_CASE("RouterTLM credit timeout configurable", "[router][vc][tlm]") {
    EventQueue eq;
    RouterTLM router("r0", &eq, 0, 0, 2, 2);
...
TEST_CASE("RouterTLM receive credit updates state", "[router][vc][tlm]") {
    EventQueue eq;
    RouterTLM router("r0", &eq, 0, 0, 2, 2);
    router.receive_credit(1, 2);
    router.tick();
    router.tick();
}

TEST_CASE("RouterTLM reverse port mapping", "[router][vc][tlm]") {
...
TEST_CASE("RouterTLM node ID computation", "[router][vc][tlm]") {
    EventQueue eq;
...
TEST_CASE("RouterTLM coord to node", "[router][vc][tlm]") {
