...
TEST_CASE("AriRouter: default ARI disabled, legacy BDF parsing unchanged",
          "[pcie][sriov][ari]") {
...
TEST_CASE("AriRouter: ARI enabled maps 8-bit function number to slot",
          "[pcie][sriov][ari]") {
    AriRouter router;
...
TEST_CASE("AriRouter: ARI function number ignores bus/dev bits",
          "[pcie][sriov][ari]") {
    AriRouter router;
    router.set_ari_enabled(true);
...
TEST_CASE("AriRouter: toggle ARI enabled via config space bit",
          "[pcie][sriov][ari]") {
    AriRouter router;
...
TEST_CASE("AriRouter: invalid function number rejected when ARI enabled",
          "[pcie][sriov][ari]") {
    AriRouter router;
    router.set_ari_enabled(true);
