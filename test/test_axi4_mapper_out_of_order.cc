...
TEST_CASE("Axi4Mapper: multiple outstanding reads, OOO rdata via rid association", "[axi][mapper][ooo]") {
    Axi4Mapper mapper(8);
    
    // Issue 3 outstanding reads: A=0x1000 (addr=0x1000), B=0x2000 (addr=0x2000), C=0x3000 (addr=0x3000)
    ...
    // Downstream returns rdata OUT OF ORDER: C first (rid=0xCCCC), then A (rid=0xAAAA), then B (rid=0xBBBB)
    // Complete C first
...
TEST_CASE("Axi4Mapper: OOO completion does not affect other outstanding (non-match not consumed)", "[axi][mapper][ooo]") {
    Axi4Mapper mapper(8);
...
TEST_CASE("Axi4Mapper: burst read multi-beat OOO (rlast only releases on last beat)", "[axi][mapper][ooo]") {
    Axi4Mapper mapper(8);
...
TEST_CASE("Axi4Mapper: pending_read returns original araddr for OOO association", "[axi][mapper][ooo]") {
    Axi4Mapper mapper(8);
