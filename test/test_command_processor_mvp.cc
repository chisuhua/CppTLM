...
TEST_CASE("CommandProcessor: set_dispatch_target stores target", "[command-processor][mvp]") {
    CommandProcessor cp;
    REQUIRE_NOTHROW(cp.set_dispatch_target(
        [](const TmuDispatchRecord&) { return TmuSubmitResult::SUBMITTED; }));
}
...
TEST_CASE("CommandProcessor: 5-transition FSM with SUBMITTED dispatch",
    ...
    cp.set_vram_reader(
        [&vram](uint64_t va, void* out, size_t sz) { return vram.read(va, out, sz); });

    DispatchSpy spy;
    cp.set_dispatch_target([&spy](const TmuDispatchRecord& rec) { return spy(rec); });
    ...
    REQUIRE(vram.call_count ==
            2); // FETCH + DECODE 各调一次 (per design §3.1,DECODE 也调以解析 header)
    REQUIRE(cp.cp_backoff_count() == 0);
    REQUIRE(cp.degraded() == false);
}

// ── DISPATCH 返回 BACKPRESSURED → 退避 ──
TEST_CASE("CommandProcessor: BACKPRESSURED dispatch increments backoff counter",
          "[command-processor][mvp]") {
    CommandProcessor cp;
    cp.set_decoder(std::make_unique<Pm4Decoder>());
...
TEST_CASE("CommandProcessor: 3 consecutive BACKPRESSURED enters DEGRADED state",
          "[command-processor][mvp]") {
    CommandProcessor cp;
    cp.set_decoder(std::make_unique<Pm4Decoder>());
...
TEST_CASE("CommandProcessor: SUBMIT_QUEUE_REJECTED also triggers backoff",
          "[command-processor][mvp]") {
    CommandProcessor cp;
    cp.set_decoder(std::make_unique<Pm4Decoder>());
...
TEST_CASE("CommandProcessor: DEGRADED gates DISPATCH (returns to FETCH only)",
          "[command-processor][mvp]") {
    CommandProcessor cp;
    cp.set_decoder(std::make_unique<Pm4Decoder>());
