...
TEST_CASE("PcieSriovVfPool: dispatch_tlp routes to per-VF config space",
    ...
    bundles::PcieTlpBundle tlp_pf(bundles::PcieTlpBundle::CFG_WRITE, 0,
                                  0x04, 4, 0x00000006, 0x0000, 1);
    REQUIRE(pool.dispatch_tlp(0, tlp_pf) == true);
    REQUIRE(pool.config_of(0).read(0x04) == 0x00000006u);

    // stream_id=1 → VF0 config write
    bundles::PcieTlpBundle tlp_vf0(bundles::PcieTlpBundle::CFG_WRITE, 0,
                                   0x04, 4, 0x00000002, 0x0000, 2);
...
TEST_CASE("PcieSriovVfPool: dispatch_tlp rejects out-of-range stream_id",
    ...
    bundles::PcieTlpBundle tlp(bundles::PcieTlpBundle::CFG_READ, 0,
                               0x04, 4, 0, 0x0000, 3);
...
TEST_CASE("PcieSriovVfPool: stream_id 16 maps to VF15 (last VF)",
    ...
    bundles::PcieTlpBundle tlp(bundles::PcieTlpBundle::CFG_WRITE, 0,
                               0x04, 4, 0x00000005, 0x0000, 9);
