...
TEST_CASE("PcieSriovVfPool: flr_vf resets target FC token bucket to capacity",
    ...
    for (int i = 0; i < 5; ++i) {
        REQUIRE(pool.fc_engine().bucket(1).consume(
            FcTokenBucket::Type::NonPosted) == true);
    }
    REQUIRE(pool.fc_engine().bucket(1).token_count(
                FcTokenBucket::Type::NonPosted) == CAP - 5);
    // PF 桶未动
    REQUIRE(pool.fc_engine().bucket(0).token_count(
                FcTokenBucket::Type::NonPosted) == CAP);
...
TEST_CASE("PcieSriovVfPool: flr_pf resets all 17 FC token buckets",
          "[pcie][sriov][vf-pool][flr]") {
    PcieSriovVfPool pool;
    constexpr uint32_t CAP = 256;
    ...
    for (uint16_t sid = 0; sid < 17; ++sid) {
        REQUIRE(pool.fc_engine().bucket(sid).consume(
            FcTokenBucket::Type::Posted) == true);
    }
    pool.flr_pf();
    for (uint16_t sid = 0; sid < 17; ++sid) {
        REQUIRE(pool.fc_engine().bucket(sid).token_count(
                    FcTokenBucket::Type::Posted) == CAP);
    }
}

TEST_CASE("PcieSriovVfPool: flr resets per-VF seq counter",
          "[pcie][sriov][vf-pool][flr]") {
    PcieSriovVfPool pool;
...
TEST_CASE("PcieSriovVfPool: flr_vf rejects invalid vf_id",
          "[pcie][sriov][vf-pool][flr]") {
    PcieSriovVfPool pool;
