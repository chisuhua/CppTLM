...
TEST_CASE("SM 顶层 + 12 子模块在 ModuleFactory 注册", "[sm-module-register][sm-microarch]") {
    ...
    std::vector<std::string> expected = {
        ...
        "FetchUnitTLM", "DecodeUnitTLM", "IssueUnitTLM",
        "ScalarALU", "VectorALU", "MatrixCore",
        "SIMTLane",
        "LsuGlobal", "LsuLDS",
        "RegFileUnit", "WritebackUnit", "HazardTracker",
    };
...
TEST_CASE("StreamingMultiprocessorTLM 直接实例化 + get_module_type", "[sm-module-register][sm-microarch]") {
    EventQueue eq;
...
TEST_CASE("13 SM 类型 ModuleFactory 注册总计 (sanity check)", "[sm-module-register][sm-microarch]") {
    auto types = ModuleFactory::getRegisteredObjectTypes();
    int sm_count = 0;
    for (const auto& name : types) {
        if (name == "StreamingMultiprocessorTLM" ||
            name == "FetchUnitTLM" || name == "DecodeUnitTLM" || name == "IssueUnitTLM" ||
            name == "ScalarALU" || name == "VectorALU" || name == "MatrixCore" ||
            name == "SIMTLane" ||
            name == "LsuGlobal" || name == "LsuLDS" ||
            name == "RegFileUnit" || name == "WritebackUnit" || name == "HazardTracker") {
            ++sm_count;
        }
    }
    REQUIRE(sm_count == 13);
