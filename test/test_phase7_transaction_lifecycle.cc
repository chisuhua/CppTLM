...
TEST_CASE("T8.3: 严重错误处理", "[error][fatal][P0]") {
    ...
    SECTION("DebugTracker 记录严重错误") {
        ...
        uint64_t error_id =
            tracker.record_error(&payload, ErrorCode::COHERENCE_DEADLOCK, "Deadlock", "cache");
...
TEST_CASE("T9.4: 错误场景端到端流程", "[integration][error][P0]") {
    ...
    SECTION("Memory 地址越界错误") {
        ...
        uint64_t error_id = err_tracker.record_error(
            &req_payload, ErrorCode::TRANSPORT_INVALID_ADDRESS, "Address out of range", "memory");
    ...
    SECTION("错误从 Memory 传播到 CPU") {
        uint64_t error_id = err_tracker.record_error(&req_payload, ErrorCode::RESOURCE_BUFFER_FULL,
                                                     "Queue overflow", "crossbar");
...
TEST_CASE("T8.2: 错误跨模块传播", "[error][propagation][P1]") {
    ...
    SECTION("错误从 Cache 传播到 Crossbar") {
        // 1. Cache 检测一致性错误
        uint64_t error_id = err_tracker.record_error(&payload, ErrorCode::COHERENCE_STATE_VIOLATION,
                                                     "State mismatch in cache", "cache_l1");
    ...
    SECTION("多个模块的错误记录") {
        // 记录不同模块的错误
        uint64_t err1 =
            err_tracker.record_error(&payload, ErrorCode::RESOURCE_BUFFER_FULL, "Full", "crossbar");
        uint64_t err2 = err_tracker.record_error(&payload, ErrorCode::TRANSPORT_TIMEOUT, "Timeout",
                                                 "noc_router");
...
TEST_CASE("T8.5: 错误与交易关联", "[error][transaction][P1]") {
    ...
    SECTION("错误与交易 ID 关联") {
        ...
        // 2. 在交易上下文中记录错误
        uint64_t error_id = err_tracker.record_error(&payload, ErrorCode::COHERENCE_DEADLOCK,
                                                     "Deadlock in transaction", "cache_l1");
