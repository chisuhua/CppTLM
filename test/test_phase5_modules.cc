...
public:
    ...
    MockResettableModule(const std::string& n, EventQueue* eq) : SimObject(n, eq) {
    }
...
TEST_CASE("DebugTracker 模块集成", "[module][error]") {
    ...
    uint64_t error_id = tracker.record_error(&payload, ErrorCode::COHERENCE_STATE_VIOLATION,
                                             "Test error", "test_module");
