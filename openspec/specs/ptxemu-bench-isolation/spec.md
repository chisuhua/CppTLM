# ptxemu-bench-isolation Specification

## Purpose
Enforce ctest gate isolation between CppTLM core tests and PTX-EMU submodule's `bench/cute/` CUDA benchmarks. PTX-EMU's `bench/cute/CMakeLists.txt` lacks the `if(PTXEMU_BUILD_TESTING)` guard (unlike its `tests/` directory which IS gated), so CppTLM's `CPPTLM_WITH_PTX_EMU=ON` builds unconditionally register 5 cute benchmarks that can cold-start hang 180s+ during PTX JIT compilation. This capability documents the wrapper-level exclusion in `scripts/test/test.sh` plus the Layer-1 root-cause follow-up.
