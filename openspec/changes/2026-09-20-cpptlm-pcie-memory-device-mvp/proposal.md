...
|---------------|---------|----------------|
| `mmio_read/write`（BAR 0） | 路由到 `PcieDisplayDevice` | **新增**：同时检查 `PcieMemoryDevice`（D2 设备也用 BAR 0） |
...
- D2 的 BAR 0 与 D1 的 BAR 0 共用 DGpuBoard 的 `bar == 0` 判断，通过 device priority 区分（display 优先，memory 其后）
...
| `src/tlm/gpu/dgpu_board_shell.cc` | **改** | `mmio_read/write` 增加 BAR 2 → memory device 分支；`backdoor_read/write` 扩展到 memory backing（**不**改 ABI wrapper） |
...
| `test/test_pcie_memory_device_routing_characterization.cc` | **新** | 回归测试 0（**T0 先写**）：锁定当前 board 路由行为，确保 D2 改动不引入回归 |
| `test/CMakeLists.txt` | **改** | 注册新增 4 个 test_*.cc 文件 |
| `docs/pcie/memory-device-mvp.md` | **新** | CppTLM 仓内实现笔记 |
...
- `PcieDisplayDevice`（D1 已交付，✅）— D2 复用其 BAR 0 路由框架
- `DGpuBoard`（✅，T3 修改扩展 BAR 2）
...
| 风险 | 等级 | 缓解策略 |
|------|------|---------|
| **R1**: BAR 0 路由 priority（display vs memory 冲突） | 🟡 中 | display_device 优先检查（`has_display_device()` 先于 `has_memory_device()`） |
| **R2**: 8GB memory backing 分配影响启动时间 | 🟢 低 | lazy alloc（首次 BAR 2 访问时分配） |
| **R3**: D2 tick() 与 D1 tick() 并行推进（无 MSI-X 但仍有 cycle counter） | 🟢 低 | memory device tick() 仅推进 cycle counter，不触发中断 |
...
- T3: DGpuBoard 路由层（mmio_read/write + backdoor_read/write BAR 2 → device）
- T4: 单元测试（basic + backing + routing 3 个）+ E2E 测试
- T5: docs/pcie/memory-device-mvp.md + AGENTS.md 更新 + 提交

## 关联

- **上游 openspec**: `2026-09-20-cpptlm-pcie-display-io-mvp`（D1，已建立路由框架）
- **后续 openspec**:
