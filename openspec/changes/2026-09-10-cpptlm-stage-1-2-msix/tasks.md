...
### 任务 1.1: 写失败测试
- [x] **Write test**: `test/test_dgpu_msix.cc::test_msix_intr_cb_called_within_200ms`
  - mock intr_cb，msix_init + trigger_irq_async(vector=0, payload=0xDEADBEEF)
  - 等待 200ms，检查 intr_cb_called ≥ 1
- [x] **Verify fail**: 当前 trigger_irq_async 返 -ENOSYS，cb_called = 0
...
- [x] **Modify**: `src/abi/cpptlm_emulator.cc`
  - 验证 `register_callbacks` 后 intr_cb 路径完整
  - msix_init 后立即可用 trigger_irq_async
- [x] **Verify pass**: 200ms 内 intr_cb ≥1 次触发，payload = 0xDEADBEEF

### 任务 1.3: Commit
- [x] **Commit (CppTLM)**: `fix(cpptlm): intr_cb 真实触发 + trigger_irq_async 接线 (修复 #4 MSI-X)`
- [ ] **Commit (UE entry sync)**: `docs(pcie-ep): v0.5 stage 1.2 MSI-X ship`
- [x] **Commit (CppTLM 18-doc mirror)**: `docs(cpptlm): v0.5.4 mirror UE stage 1.2 MSI-X ship`
...
### 任务 2.1: 1 次轻量复审
- [x] **Oracle review**: intr_cb 真实触发 + 修复 #4 闭环验证
- [x] **Verify**: `ctest -R "dgpu|msix"` 全 PASS（既有 ABI 测试零回归）
...
- [x] **Modify**: `src/tlm/pcie/pcie_msix_per_vf_tlm.cc`
  - 实现 MSI-X Extended Capability 结构
  - 至少 4-8 个中断向量表项
- [x] **Write test**: `test/test_dgpu_msix_cap.cc::test_msix_cap_vector_count_4_to_8`
  - 验证 Cap 存在 + table_size ∈ [4, 8]
- [x] **Verify fail → implement → Verify pass**: 表项可独立 mask/unmask
- [x] **Commit (CppTLM)**: `feat(cpptlm): MSI-X Cap 4-8 vectors (基础 1.2.2)`

### 任务 2.5.2：中断节流 Interrupt Coalescing（基础任务 1.2.3）
- [x] **Implement**: `pcie_endpoint_ip.cc` 基础中断合并机制（避免高频小任务导致中断风暴）
  - 可配置 threshold（默认 8）+ timeout（默认 50us）
- [x] **Write test**: `test/test_dgpu_msix_throttle.cc::test_msix_throttle_burst_8_in_one_intr`
- [x] **Verify fail → implement → Verify pass**: 8 次连续 trigger 合并为 1 次 intr_cb 触发
- [x] **Commit (CppTLM)**: `feat(cpptlm): MSI-X interrupt coalescing (基础 1.2.3)`
