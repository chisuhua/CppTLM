...
- **WHEN** `msix_init(4)` + `register_callbacks({intr_cb})` + `trigger_irq_async(0, 0xDEADBEEF)` are called
- **THEN** the function returns 0 (async fire-and-forget)
- **AND** `intr_cb(0, 0xDEADBEEF)` is invoked within 200ms timeout
- **AND** `intr_cb_called` count ≥ 1

#### Scenario: Out-of-range vector returns -EINVAL

- **WHEN** `trigger_irq_async(999, 0)` is called with `msix_init(4)` (table_size=4)
- **THEN** the function returns -EINVAL
- **AND** no TLP pushed to inject_q_
- **AND** intr_cb not invoked

#### Scenario: Already pending vector returns -EAGAIN

- **WHEN** `trigger_irq_async(0, X)` is called twice without drain
- **THEN** first call returns 0
- **AND** second call returns -EAGAIN
- **AND** only one intr_cb invocation (after drain)

#### Scenario: Vector 0-3 valid range

- **WHEN** `msix_init(8)` then `trigger_irq_async(vector=0..7, payload)`
