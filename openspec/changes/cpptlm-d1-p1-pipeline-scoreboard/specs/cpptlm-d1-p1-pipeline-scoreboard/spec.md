...
>
> **验收归属说明**（Oracle P0 审查后更新, 2026-07-18）:
> - **CppTLM 端可独立验证** ✅: G-D1, G-D4
> - **PTX-EMU 端已验证** ✅: G-D6, G-D7 (PTX-7a 7 Mock + PTX-7b 4 集成, 210/210 PASS, PTX-EMU injection-points change 已归档 `e3070360`)
> - **双端联合验收**（需 Phase 4 Wave 0+1 完成后）: G-D2, G-D3, G-D5, G-D8

- [x] **G-D1** [CppTLM 端] 3 pure interfaces compile and link; vendor 头文件零依赖 ✅ 2026-07-18 (16 单测 567 assertions PASS)
- [ ] **G-D2** [双端] `set_blocked_cycles_for_active()` correctly applies per-warp latency (Phase 4 Wave 2)
- [ ] **G-D3** [双端] `blocked_cycles_remaining` differs from standalone by ≤ 1 cycle (需统一 cycle 契约: 1 CppTLM tick = 1 GPUContext::exe_once())
- [x] **G-D4** [CppTLM 端] 12-endpoint static_assert passes (enum + 签名级 decltype 验证) ✅ 2026-07-18 (commit `09b64b6`, negative test 证实断言生效)
- [ ] **G-D5** [双端] 5 microbenchmarks stay within ±15% of gpgpu-sim baseline (Phase 4 Wave 2)
- [x] **G-D6** [PTX-EMU 端] 3 setter all nullptr → zero regression ✅ 2026-07-18 (PTX-7a `test_smcontext_injection.cpp`, `sm_context.cpp:32` nullptr guard)
- [x] **G-D7** [PTX-EMU 端] pipeline/TC nullptr → fallback InstructionLatencyTable ✅ 2026-07-18 (`test_step_b_set_blocked_cycles.cpp` 4-branch 回归锁, `sm_context.cpp:328`)
- [ ] **G-D8** [双端] `exe_once` stall → re-schedule → release → re-issue loop is consistent (Phase 4 Wave 2, 需真实 SMContext::exe_once() 链路)
...
| Repo | Change | Role |
|------|--------|------|
| PTX-EMU | `openspec/changes/archive/2026-07-18-cpptlm-phase8b-injection-points/` (已归档) | Upstream injection interfaces + `exe_once()` 3-step hooks + PTX-7a/7b tests (210/210 PASS) |
