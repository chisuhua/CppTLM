...
- [x] **Write test**: `test/test_pcie_power_state_cfg_access.cc`
...
- [x] **Modify**: `src/tlm/pcie/pcie_endpoint_ip.cc` tick() 入口
  - 移除早返回 `if (mmio_gated_) return;` (line 233-235)
  - 在 cfg/BAR 判别后, 仅 `mmio_gated && !is_cfg` 时返回 DECERR 响应
  - 注意 cfg 路径需在 D3hot 下保留 (INV-E)
- [x] **Verify fail → implement → Verify pass**: 5 cases 全 PASS
- [x] **Commit (CppTLM)**: `fix(pcie): INV-A gate narrowed to BAR only + DECERR response (Stage 1.4 §1)`

## §2 任务 1.2：PMCSR PWS=1/2 mask

- [x] **Write test**: **EP 级测试**（勘误: mask 在 EP 构造器 lambda `pcie_endpoint_ip.cc:28-30`，`test_pm_capability.cc` 测独立 PcieConfigSpace 够不到）— 扩展 `test_power_state_transition.cc` 或新 `test_pcie_power_state_cfg_access.cc`:
...
- [x] **Modify**: `src/tlm/pcie/pcie_endpoint_ip.cc:28`
  - `set_pmcsr_write_cb` lambda 加 mask: `if (new_pws != 0 && new_pws != 3) return;`
- [x] **Verify**: 既有 [pm] 测试全 PASS + 新 SECTION 通过
- [x] **Commit (CppTLM)**: `fix(pcie): PMCSR PWS=1/2 reserved values ignored (PCI PM spec)`

## §3 任务 1.3：PM Cap 控制域 JSON-driven

- [x] **Write test**: `test/test_pm_cap_control_json.cc`
...
- [x] **Verify**: 3 cases 全 PASS, 既有 [pm] 测试无回归
- [x] **Commit (CppTLM)**: `feat(pcie): PM Cap control word JSON-driven`

## §4 任务 2.1：ACS Extended Cap 完整实现

- [x] **Write test**: `test/test_acs_extended_cap.cc`
  - 默认安装: read offset=0xE0 → 0x000D0001 (id=0x000D, version=1)
  - ACS Cap Reg 读: 0x00000000 (no caps)
  - enable V bit (bit 0): 读 Control → 0x0001
- [x] **New file**: `src/tlm/pcie/pcie_acs_extended_cap.cc`
...
- [x] **Modify**: `src/tlm/pcie/pcie_endpoint_ip.cc` 构造期
  - 在 PM Cap 安装后调用 `install_acs_extended_cap(cfg_pf)`
- [x] **Verify**: 3 cases 全 PASS, [pcie] 全绿
- [x] **Commit (CppTLM)**: `feat(pcie): ACS Extended Cap (id=0x000D) 完整实现`

## §5 任务 3.1：ResizableBar → PcieEndpointIP 集成

- [x] **Write test**: `test/test_resizable_bar_integration.cc`
...
- [x] **Verify**: 3 cases 全 PASS
- [x] **Commit (CppTLM)**: `feat(pcie): ResizableBar 集成 PcieEndpointIP + INV-G 越界校验`

## §6 任务 4.1：P2P DMA SUCCESS 路径 + AcsPolicy

- [x] **Write test**: `test/test_p2p_dma_success.cc`
  - 默认 AcsPolicy: 跨 BDF BLOCKED_BY_ACS (向后兼容)
  - `grant(src, dst)` 后: SUCCESS
  - 不同 BDF pair grant 不影响其他
  - 1000-iter fuzz + 多 pair grant
- [x] **Modify**: `include/tlm/pcie/pcie_bypass_mux_p2p.hh`
  - 新增 `AcsPolicy` 类 (grant 集合)
  - `p2p_dma_route` 增可选 `const AcsPolicy*` 参数 (default nullptr)
  - policy 非空且 !allow → BLOCKED_BY_ACS; 否则 SUCCESS
- [x] **Verify**: 4 cases / 2000+ assertions PASS, 既有 [p2p] 测试无回归
- [x] **Commit (CppTLM)**: `feat(pcie): P2P DMA SUCCESS path via AcsPolicy (Stage 2.1 §2.1 完整实现)`
