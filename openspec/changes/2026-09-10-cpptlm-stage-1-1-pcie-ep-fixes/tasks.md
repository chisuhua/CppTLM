...
### 任务 1.1.1.1: 写失败测试
- [x] **Write test**: `test/test_dgpu_pcie_config.cc::test_pcie_config_read_vendor_id_returns_0x10DE`
- [x] **Verify fail**: 当前返 -ENOSYS

### 任务 1.1.1.2: 实施转发
- [x] **Modify**: `src/tlm/gpu/dgpu_board_shell.cc:151-164`
...
### 任务 1.1.1.3: Commit
- [x] **Commit**: `fix(dgpu-board): forward pcie_config_read/write to cfg_space (修复 #3)`

## §2 阶段 1.1.2: 修复 #6 (backdoor_read miss 返 -ENOENT)

### 任务 1.1.2.1: 写失败测试
- [x] **Write test**: `test/test_dgpu_backdoor.cc::test_backdoor_read_miss_returns_enoent_not_len`
- [x] **Verify fail**: 当前 miss 返 `len` (例如 4)

### 任务 1.1.2.2: 实施返值修正
- [x] **Modify**: `src/tlm/gpu/dgpu_board_shell.cc:168-190`
  - 加 buf nullptr 检查
  - miss 返 `-ENOENT`（-38）
...
### 任务 1.1.2.3: Commit
- [x] **Commit**: `fix(dgpu-board): backdoor_read miss returns -ENOENT (修复 #6)`

## §3 阶段 1.1.3: 修复 #5 (mmio_read 数据拷贝)

### 任务 1.1.3.1: 写失败测试
- [x] **Write test**: `test/test_dgpu_mmio.cc::test_mmio_read_real_data_not_garbage`
- [x] **Verify fail**: 当前 buf 未填充，data 是 garbage

### 任务 1.1.3.2: 实施数据拷贝
- [x] **Modify**: `src/tlm/gpu/dgpu_board_shell.cc:106-133`
...
- [x] **Modify**: `src/tlm/pcie/pcie_endpoint_ip.cc` (sim_loop drain 实现)
- [x] **Verify pass**: 测试通过（buf 真实填充）

### 任务 1.1.3.3: Commit
- [x] **Commit**: `fix(dgpu-board): mmio_read real data copy (修复 #5)`

## §4 阶段 1.1.4: 修复 #7 (mmio_write 文档矛盾澄清)

### 任务 1.1.4.1: 修改 design.md
- [x] **Modify**: `openspec/changes/2026-09-09-cpptlm-pcie-ep-foundation/design.md §3.1`
...
- [x] **Verify**: `architecture/18-pcie-endpoint-entry.md §2.1` 已说 "MMIO Write 是异步"
- [x] **Verify**: 代码 `dgpu_board_shell.cc:148` 返 0 立即返回

### 任务 1.1.4.3: Oracle 轻量复审
- [x] **Oracle review**: 1 次轻量复审，确认保持 async 裁决
- [x] **Commit**: `docs(cpptlm): clarify mmio_write async semantics (修复 #7)`

## §5 集成测试

### 任务 5.1: 完整 PCIe EP 基础测试套件
- [x] **Run**: `./build/bin/cpptlm_tests "[dgpu][pcie]"` — 4 修复全部测试通过
- [x] **Run**: `./build/bin/cpptlm_tests "[abi]"` — 既有 ABI 测试零回归
- [x] **Verify**: 152/152 tests PASS (新增 4 测试)
...
- [x] **Modify**: `test/test_dgpu_board_shell_abi.cc`
...
- [x] **Verify**: 更新后 test_dgpu_board_shell_abi PASS（既有 ABI 测试**预期**因修复 #6 行为变更而调整，非"零回归"）

## §6 Oracle Gate E 复审（v1.0 → v1.1 升档）

### 任务 6.1: Oracle 实施后复审
- [x] **Oracle review**: 1 次完整实施复审，4 修复质量评估
- [x] **Verify**: ADR-035 §R2 v1.0 → v1.1 升档流程通过

### 任务 6.2: 同步双仓 entry
- [x] **Modify**: UsrLinuxEmu `entry §11` ADR-088 / `cpptlm-pcie-ep-foundation` 关联行
- [x] **Verify**: CppTLM `18-doc §11` 同步状态
