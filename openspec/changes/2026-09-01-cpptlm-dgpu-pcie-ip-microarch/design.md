# cpptlm-dgpu-pcie-ip-microarch: 微架构设计 (Microarchitecture Design)

> **配套**: [`proposal.md`](./proposal.md) · [`decisions.md`](./decisions.md) · [`roadmap.md`](./roadmap.md)
> **状态**: 📐 Design · **Owner**: CppTLM Team
> **关联**: ADR-SOC-07 D2 + Oracle `bg_661f4ee7` + Librarian `bg_07392e0c`
...
`PcieEndpointIP` 是 dGPU SoC 片内 PCIe Endpoint IP 的完整 CppTLM 模型,目标行为接近真实硬件数字逻辑链路训练 → 链路层握手 → 事务层路由 → SoC AXI 交互全流程。
...
**关键交互注释**:
- **PIPE 接口**: 仅 4 个信号,详见 [`decisions.md` §Q3](./decisions.md)
- **AXI 接口**: 多端口(master / slave / cfg_slave),详见 §6
- **Host Bypass**: 独立组件,见 [`roadmap.md` §Phase 7](./roadmap.md)
- **PHY BFM**: 开源选项见 [`decisions.md` §Q4](./decisions.md)
...
**状态**: 📐 Design — 等待 OpenSpec 评审后,Phase 1 子 change 启动
