...
- [ ] **Write test**: `test/test_pcie_endpoint_ip_simmodule_refactor.cc::test_composite_construction_order`
...
- [ ] **Create**: `include/tlm/pcie/pcie_link_phy_mux_tlm.hh`
...
- [ ] **Verify**: `./build/bin/cpptlm_tests "[pcie-simmodule-refactor][composite]"` 全绿
...
- [ ] **Write test**: `test_pcie_endpoint_ip_simmodule_refactor.cc::test_composite_tick_order_phase1`
...
- [ ] **Write test**: `test_pcie_endpoint_ip_simmodule_refactor.cc::test_ep_tick_does_not_drive_phy_phase1`  ← **新增兼容性锁定测试**
...
- [ ] **Create**: `src/tlm/pcie/pcie_link_phy_mux_tlm.cc`
...
- [ ] **Write test**: `test_pcie_endpoint_ip_simmodule_refactor.cc::test_static_registry_shim`
...
- [ ] **Write test**: `test_pcie_endpoint_ip_simmodule_refactor.cc::test_json_consumption_7keys`
...
- [ ] **Modify**: `include/tlm/pcie/pcie_link_layer_tlm.hh`(proposal §Impact 例外)
...
- [ ] **Modify**: 4 个子模块静态注册表 `for_endpoint` 实现为 composite-first + fallback（per `design.md §2.3`）
...
- [ ] **Commit (CppTLM)**: `feat(pcie): introduce PcieLinkPhyMuxTLM composite + static-registry shim (phase-1 dormant tick)`

### 任务 1.4 — Phase 1 全量回归 + Oracle gate

- [ ] **Run**: `./test.sh --mode off --quick`（含 `[pcie]` Catch2 全量 + Python demo）
- [ ] **Run**: `python3 examples/demo_pcie_full_e2e.py`
- [ ] **Run**: `openspec validate 2026-09-15-cpptlm-pcie-endpoint-ip-simmodule-refactor --strict`（提案文件结构校验）
- [ ] **Verify**:
  - 44498 assertions 全绿
  - 14+ 注册表依赖测试文件零修改
...
- [ ] **Write test**: `test_pcie_endpoint_ip_simmodule_refactor.cc::test_ep_inheritance_changed`
...
- [ ] **Modify**: `include/tlm/pcie/pcie_link_phy_mux_tlm.hh`
...
- [ ] **Modify**: 4 个子模块 `for_endpoint` 实现更新(`.cc` only, `.hh` 布局不变):
...
- [ ] **Verify**:
  ...
  - 44498 assertions 全绿
- [ ] **Commit (CppTLM)**: `refactor(pcie): switch PcieEndpointIP base from ChStreamModuleBase to SimModule`
...
- [ ] **Audit**: `grep -rn "phy.*->tick()\|phy.*\.tick()" test/ | grep -v simmodule_refactor`
  - 列出所有外部驱动 PHY tick 的测试 fixture(预期:test_aspm.cc / test_link_training*.cc / test_pcie_endpoint_ip_*.cc 等)
- [ ] **Modify**: 命中的测试文件,移除 EP 创建后的外部 `phy->tick()` 循环驱动;改为单次 `setup_phy_state()` 推 PHY 至目标 LTSSM state(若 LTSSM 推进依赖 tick,改为 `composite->phy().tick()` 或 `composite->tick()` 局部驱动)
  - **允许**:测试 fixture 内部 `composite_->tick()` 调用(经 `PcieLinkPhyMuxTLM::for_endpoint(ep_name)` 取 composite)
  - **禁止**:重新引入对 EP::tick 内部细节的依赖
- [ ] **Write test**: `test_pcie_endpoint_ip_simmodule_refactor.cc::test_ep_tick_drives_phy_phase2`
  - ASSERT: Phase 2 后 `ep.tick()` 推进 PHY LTSSM 状态(原 Phase 1 测试断言方向**翻转**)
- [ ] **Verify**: `[pcie-simmodule-refactor][phy-migration]` + `[pcie][aspm]` + `[pcie][link-training]` 全绿
- [ ] **Commit (CppTLM, 合并入 2.1 主 commit)**: `test(pcie): migrate PHY tick drivers from external to EP::tick (phase-2)`
...
- [ ] **Write test**: `test_pcie_endpoint_ip_simmodule_refactor.cc::test_composite_internal_reachability_17`
...
- [ ] **Verify**:
  ...
  - 17 端口程序化可达性全绿
- [ ] **Commit (CppTLM)**: `feat(pcie): composite 17-port internal reachability via getInternalOutputPort (no outer JSON wiring)`
...
- [ ] **Write test**: `test_pcie_endpoint_ip_simmodule_refactor.cc::test_axislavein_bridge_path_intact`
...
- [ ] **Run**: `ctest --test-dir build --output-on-failure -j4`（CMake 注册的测试集）
- [ ] **Run**: `./build/bin/cpptlm_tests "[pcie]"`（全量 Catch2 44498 assertions）
- [ ] **Run**: `./build/bin/cpptlm_tests "[pcie-simmodule-refactor]"`（新增 6+ TEST_CASE）
- [ ] **Verify**:
  - E2E PASS
  - 44498 assertions 全绿
  - 6+ 新增 TEST_CASE 全绿
...
- [ ] **Run**: `openspec validate 2026-09-15-cpptlm-pcie-endpoint-ip-simmodule-refactor --strict`
- [ ] **Run**: `git diff --stat`（与 Impact 章节列出的修改/新增文件清单对比）
- [ ] **Verify**:
...
- [ ] **Submit Oracle 复评证据**（per `proposal.md §Oracle 复评` 7 项清单）
...
- [ ] **Modify**: `docs/architecture/14-pcie-ip-microarchitecture.md`
...
- [ ] **Modify**: `include/AGENTS.md` 注册宏体系表
...
- [ ] **Compile evidence**（per `proposal.md §Oracle 复评` 7 项清单）:
  1. `git diff --stat` 输出
...
- [ ] **Submit**: Oracle 复评 evidence package
- [ ] **Await**: Oracle 复评 PASS 后 archive
