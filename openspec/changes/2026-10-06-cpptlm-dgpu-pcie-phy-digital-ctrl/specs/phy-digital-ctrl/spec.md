# phy-digital-ctrl Spec — Phase 3 (PHY Digital + Bypass Mux + Rate Switch)
...
> **Oracle 评审 C4 补交**: Phase 3 原交付缺 spec.md,本文件补齐 3 个 ADDED Requirement 的 delta 格式

## Purpose

本 spec 定义 Phase 3 交付的 3 个 ADDED Requirement—— `PHY_DIGITAL_CONTROL` (LTSSM 11 态训练序列 + 低功耗 + Disabled/Loopback/Hot_Reset 可达)、`BYPASS_MUX_3_MODE` (Full/Bypass/Partial 三态数据路径)、`RATE_SWITCH_INTEGRATION` (速率切换完成后编码延迟模型同步新速率)。
