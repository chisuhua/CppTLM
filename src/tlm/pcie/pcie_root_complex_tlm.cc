...
uint32_t PcieRootComplexTLM::config_read(uint16_t device, uint16_t function,
                                         uint16_t offset, uint16_t stream_id) {
...
bool PcieRootComplexTLM::config_write(uint16_t device, uint16_t function,
                                      uint16_t offset, uint32_t value, uint16_t stream_id) {
    if (!ep_ || stream_id >= PcieEndpointIP::NUM_PORTS) {
        return false;
    }
    (void)device;
...
bool PcieRootComplexTLM::bar_allocate(uint16_t device, uint16_t function,
                                      uint16_t bar_offset, uint32_t value) {
    if (!ep_) {
        return false;
    }
    (void)device;
...
bool PcieRootComplexTLM::bar_write(uint16_t device, uint64_t addr, uint64_t data, uint8_t bytes) {
    if (!ep_) {
        return false;
    }
    (void)device;
...
bool PcieRootComplexTLM::bar_read(uint16_t device, uint64_t addr, uint64_t& data, uint8_t bytes) {
    if (!ep_) {
        return false;
    }
    (void)device;
