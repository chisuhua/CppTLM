...
uint64_t PcieEncodingLatencyModel::block_latency_ns(
    Rate rate, std::size_t block_bytes, std::size_t active_lanes) noexcept {
...
uint32_t PcieEncodingLatencyModel::gen_index(Rate rate) noexcept {
    switch (rate) {
    case Rate::GEN1: return 0u;
    case Rate::GEN2: return 1u;
    case Rate::GEN3: return 2u;
    case Rate::GEN4: return 3u;
    case Rate::GEN5: return 4u;
    }
    return 0u;
}

uint64_t PcieEncodingLatencyModel::rate_switch_delay_us(
    Rate from, Rate to) noexcept {
    // 同速率: 无链路重训练 → 0µs
    if (from == to)
        return 0u;
