...
void MemoryCluster::set_config(const nlohmann::json& params) {
    ...
    if (params.contains("channel_count")) channel_count_ = params["channel_count"].get<int>();
    if (params.contains("channel_size")) channel_size_ = params["channel_size"].get<std::string>();
    if (params.contains("memory_type")) memory_type_ = params["memory_type"].get<std::string>();
}
