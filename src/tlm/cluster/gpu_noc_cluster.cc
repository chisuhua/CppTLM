...
void GpuNoC::set_config(const nlohmann::json& params) {
    ...
    if (params.contains("mesh_size")) mesh_size_ = params["mesh_size"].get<int>();
    if (params.contains("routing")) routing_ = params["routing"].get<std::string>();
}

void GpuNoC::simulate_instantiate(const nlohmann::json& cfg) {
    SimModule::simulate_instantiate(cfg);
    nlohmann::json full_config = {{"modules", nlohmann::json::array()}, {"connections", nlohmann::json::array()}};
    int n = mesh_size_ * mesh_size_;
    for (int i = 0; i < n; ++i) {
        full_config["modules"].push_back({
            {"name", "router_" + std::to_string(i)}, {"type", "RouterTLM"},
            {"params", {{"node_x", i % mesh_size_}, {"node_y", i / mesh_size_},
                         {"mesh_x", mesh_size_}, {"mesh_y", mesh_size_},
                         {"routing", routing_}}}
        });
    }
