...
namespace tlm {
    namespace {
        ...
        //   fma / add.f32 / mul.f32 / sub.f32 / div / rcp.approx (非 SFU) / abs/neg (f32 variants)
        //   → 4.22 mad / mul.lo / mul.hi / sad → 2.0
        ...
        double latency_p3(const std::string& instr) {
            if (has(instr, "ld.global") || has(instr, "ld.volatile"))
                return 200.0;
            if (has(instr, "st.global"))
                return 20.0;
            if (has(instr, "ld.shared") || has(instr, "st.shared"))
                return 1.0;
            if (has(instr, "ld.local") || has(instr, "st.local"))
                return 5.0;
            if (has(instr, "atom") || has(instr, "red."))
                return 200.0;
            if (has(instr, "ld."))
                return 200.0;
            if (has(instr, "st."))
        ...
        double latency_p2(const std::string& instr) {
            if (has(instr, "sin") || has(instr, "cos") || has(instr, "tan"))
                return 16.0;
            if (has(instr, "rcp") || has(instr, "rsqrt"))
                return 4.0;
            if (has(instr, "sqrt"))
                return 8.0;
            if (has(instr, "lg2") || has(instr, "ex2"))
                return 8.0;
            return 8.0;
        }
    ...
    double PipelineTLM::get_fractional_cycles(const std::string& instruction,
        ...
        switch (pipe_id) {
        case PipelineId::P0_INT_FP32:
            return latency_p0(instruction);
        case PipelineId::V_SIMD:
            return 1.0;
        case PipelineId::P1_FP64:
            return latency_p1(instruction);
        case PipelineId::P2_SFU:
            return latency_p2(instruction);
        case PipelineId::P3_LSU:
            return latency_p3(instruction);
        case PipelineId::P4_TC:
            return 0.0;
        }
        return 1.0;
    }
