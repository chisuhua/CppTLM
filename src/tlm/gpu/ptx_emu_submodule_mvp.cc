...
namespace tlm {
    ...
    // RAII — ctor / dtor
    // =============================================================================
    PtxEmuSubmoduleMVP::PtxEmuSubmoduleMVP() : impl_(std::make_unique<Impl>()) {
    }
    ...
    bool PtxEmuSubmoduleMVP::set_next_pc(uint32_t sm_id, uint32_t warp_id, uint32_t lane_id,
                                         uint32_t pc) {
        if (!impl_ || !impl_->device)
            return false;
    ...
    // =============================================================================
    ThreadState PtxEmuSubmoduleMVP::get_thread_state(uint32_t sm_id, uint32_t warp_id,
                                                     uint32_t lane_id) {
        if (!impl_ || !impl_->device)
    ...
    void PtxEmuSubmoduleMVP::attach_timing(void* scoreboard, void* pipeline, void* tensor_core) {
        if (!impl_ || !impl_->device)
            return;
        impl_->device->attach_timing(reinterpret_cast<ptxemu::IScoreboard*>(scoreboard),
    ...
    // =============================================================================
    uint32_t PtxEmuSubmoduleMVP::get_active_mask_from_warp_status(uint32_t sm_id,
                                                                  uint32_t warp_id) {
    ...
    int32_t PtxEmuSubmoduleMVP::get_blocked_cycles_from_warp_status(uint32_t sm_id,
                                                                    uint32_t warp_id,
                                                                    uint32_t lane_id) {
        WarpStatus ws = get_warp_status(sm_id, warp_id);
        if (lane_id < ws.lanes.size()) {
            ...
            if (ws.lanes[lane_id].state == ThreadState::kBarSync)
                return 1;
        }
        return 0;
    }
    ...
    bool PtxEmuSubmoduleMVP::is_warp_finished_from_warp_status(uint32_t sm_id, uint32_t warp_id) {
        ...
        for (const auto& lane : ws.lanes) {
            if (lane.state != ThreadState::kExit)
                return false;
        }
        return true;
    }
    ...
    // =============================================================================
    bool PtxEmuSubmoduleMVP::read_register_u32(uint32_t, uint32_t, uint32_t, const char*,
                                               uint32_t& out_value) {
        out_value = 0;
        return false; // PTX-EMU 端未暴露 register 读写
    }
    bool PtxEmuSubmoduleMVP::read_register_u64(uint32_t, uint32_t, uint32_t, const char*,
                                               uint64_t& out_value) {
        out_value = 0;
        return false;
    }
    bool PtxEmuSubmoduleMVP::write_register_u32(uint32_t, uint32_t, uint32_t, const char*,
                                                uint32_t) {
        return false;
    }
    bool PtxEmuSubmoduleMVP::write_register_u64(uint32_t, uint32_t, uint32_t, const char*,
                                                uint64_t) {
        return false;
    }
    ...
    PtxEmuSubmoduleMVP::decode_ptxir(const std::vector<uint8_t>& /*ptxir_bytes*/) {
