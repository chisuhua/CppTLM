...
namespace tlm {
    ...
    void CudaCoreAdapterMVP::inject_timing_modules() {
        ...
        warp_scheduler_ =
            std::make_unique<MinimalWarpSchedulerTLM>("warp_scheduler", /*EventQueue=*/nullptr);
        ...
        facade_->attach_timing(static_cast<void*>(scoreboard_.get()),
    ...
    bool CudaCoreAdapterMVP::on_cta_arrival(const CtaDescriptor& cta) {
        ...
        if (cta.shared_mem_size > 48ULL * 1024)
            return false; // 单 SM shared mem 上限 (HSK-8 默认)
        if (cta.warp_count > 64)
            return false; // 单 SM warp 上限 (HSK-8 默认)
        return true;
    }
    ...
    // =============================================================================
    CudaCoreAdapterMVP::WarpStateMirror CudaCoreAdapterMVP::get_warp_state(uint32_t sm_id,
                                                                           uint32_t warp_id) const {
