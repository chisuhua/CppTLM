...
GpuSocTLM::GpuSocTLM(const std::string& name, EventQueue* eq)
    : SimModule(name, eq) {}

void GpuSocTLM::tick() {
    // 递归推进所有子模块
    if (gpu_cluster_)    gpu_cluster_->tick();
    if (noc_)            noc_->tick();
    if (memory_cluster_) memory_cluster_->tick();
    if (kernel_launch_)  kernel_launch_->tick();
}
