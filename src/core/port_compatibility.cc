...
namespace cpptlm {
    ...
    std::string PortCompatibility::get_incompatibility_reason(const PortSpec& src,
                                                              const PortSpec& dst) {
        std::string reason;
    ...
    const char* PortCompatibility::role_name(PortRole role) {
        switch (role) {
        case PortRole::INITIATOR:
            return "initiator";
        case PortRole::TARGET:
            return "target";
        case PortRole::BI_DIRECTIONAL:
            return "bi_directional";
        case PortRole::NETWORK:
            return "network";
        case PortRole::PE:
            return "pe";
        default:
    ...
    const char* PortCompatibility::bundle_name(BundleType bundle) {
        switch (bundle) {
        case BundleType::CACHE_REQ:
            return "cache_req";
        case BundleType::CACHE_RESP:
            return "cache_resp";
        case BundleType::NOC_FLIT:
            return "noc_flit";
    ...
    bool PortCompatibility::check_role_matrix(PortRole src, PortRole dst) {
        // 使用静态二维数组实现兼容性矩阵
        static const bool role_matrix[5][5] = {// INITIATOR  TARGET  BI_DIR  NETWORK PE
    ...
    bool PortCompatibility::check_bundle_matrix(BundleType src, BundleType dst) {
        static const bool bundle_matrix[4][4] = {// CACHE_REQ  CACHE_RESP  NOC_FLIT  GENERIC
