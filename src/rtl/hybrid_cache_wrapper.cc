...
namespace cpptlm {
    namespace rtl {
        ...
        public:
            ...
            HybridCacheWrapperImpl() : device_(), simulator_(device_.context(), false) {
            }
            ...
            void tick() {
                ...
                if (pending_tx_) {
                    const TransactionContextExt* ext =
                        get_transaction_context(pending_tx_->payload);
                    ...
                    if (io.resp_out.valid) {
                        ...
                        resp_beat.tid = static_cast<uint64_t>(
                            simulator_.get_value(io.resp_out.payload.transaction_id));
                        resp_beat.parent_id = static_cast<uint64_t>(
                            simulator_.get_value(io.resp_out.payload.parent_id));
                        resp_beat.fragment_id = static_cast<uint8_t>(static_cast<uint64_t>(
                            simulator_.get_value(io.resp_out.payload.fragment_id)));
                        resp_beat.fragment_total = static_cast<uint8_t>(static_cast<uint64_t>(
                            simulator_.get_value(io.resp_out.payload.fragment_total)));
                        resp_beat.data =
                            static_cast<uint64_t>(simulator_.get_value(io.resp_out.payload.data));
                        resp_beat.hit = static_cast<bool>(static_cast<uint64_t>(
                            simulator_.get_value(io.resp_out.payload.is_hit)));
                        resp_beat.error_code = static_cast<uint8_t>(static_cast<uint64_t>(
                            simulator_.get_value(io.resp_out.payload.error_code)));
                        resp_beat.first = static_cast<bool>(
                            static_cast<uint64_t>(simulator_.get_value(io.resp_out.payload.first)));
                        resp_beat.last = static_cast<bool>(
                            static_cast<uint64_t>(simulator_.get_value(io.resp_out.payload.last)));
        ...
        HybridCacheWrapper::HybridCacheWrapper(const std::string& name, EventQueue* eq)
            : ChStreamModuleBase(name, eq), impl_(std::make_unique<HybridCacheWrapperImpl>()) {
        }
