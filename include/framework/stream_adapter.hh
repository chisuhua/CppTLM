...
namespace cpptlm {
    ...
    public:
        ...
        bool process(Packet* pkt) {
            ...
            bool ok = bundles::deserialize_bundle(pkt->payload->get_data_ptr(),
                                                  pkt->payload->get_data_length(), stream_data_);
    ...
    public:
        ...
        bool send(MasterPort* port, PacketType type = PKT_RESP, uint64_t stream_id = 0) {
            ...
            bool ok = bundles::serialize_bundle(stream_data_, pkt->payload->get_data_ptr(),
    ...
    public:
        ...
        void bind_ports(MasterPort* req_out, SlavePort* resp_in, MasterPort* resp_out = nullptr,
                        SlavePort* req_in = nullptr) override {
        ...
        void tick() override {
            // 请求方向：模块产生请求 → 序列化 → 通过 MasterPort 发送
            if (module_->req_out().valid()) {
                if (req_out_port_) {
                    module_->req_out().send(req_out_port_, PKT_REQ);
                }
            }
            // 响应方向：模块有响应数据 → 序列化 → 通过 MasterPort 发送
        ...
        void process_request_input(Packet* pkt) override {
            ...
            // P0-5b: 按 pkt->type 路由到正确 stream
            //   PKT_RESP → 响应 → module_->resp_in()（initiator 接收响应）
            //   PKT_REQ  → 请求 → module_->req_in()  （responder 接收请求）
            if (pkt->type == PKT_RESP) {
                auto& resp_in = module_->resp_in();
                if (!resp_in.valid()) {
                    resp_in.process(pkt);
                }
            } else {
