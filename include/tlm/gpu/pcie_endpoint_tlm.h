...
namespace tlm::gpu {
     ...
     *       {"offset": 0x0014, "name": "GPU_REG_DOORBELL", "access": "WO", "side_effect": "doorbell", "stream_id": 0},
     *       ...
     *     ],
    ...
    public:
        ...
        // 测试/调试访问器
        PcieConfigSpace& config_space() { return *cfg_space_; }
        const PcieConfigSpace& config_space() const { return *cfg_space_; }
        PcieBarRouter& bar_router() { return *bar_router_; }
        const PcieBarRouter& bar_router() const { return *bar_router_; }
        MsiXTable& msix() { return *msix_; }
        const MsiXTable& msix() const { return *msix_; }
    ...
    private:
        ...
        // 4 端口 Bundle 适配器（per port-type：3 个 PcieTlpBundle + 1 个 MsiXDeliveryBundle）
        cpptlm::InputStreamAdapter<bundles::PcieTlpBundle>   slave_in_;
        cpptlm::OutputStreamAdapter<bundles::PcieTlpBundle>  mmio_out_;
        cpptlm::OutputStreamAdapter<bundles::PcieTlpBundle>  mem_out_;
        cpptlm::OutputStreamAdapter<bundles::PcieTlpBundle>  irq_out_;  // IRQ_DELIVERY kind

        // 内部：处理 slave_in 收到的 PcieTlpBundle
