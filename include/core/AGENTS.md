...
- **Phase 1c 后状态**：`packet_to_payload.hh` 和 `payload_to_packet.hh` 已移除（零 .cc 用户，archived 到 `docs-archived/dead-code-headers-2026-q2/`）；`cmd_exts.hh` 简化为宏库（删除重复的 `ReadCmdExt`/`WriteCmdExt` 等类定义，仅 `ReqIDExt` 唯一保留）
