...
| T3.2-03 | **ModuleFactory 集成 port_specs JSON 反序列化** | 2d | T3.2-01 | JSON 配置端口规格解析正确 |
| T3.2-04 | **端口方向检查 (L1)** — bindPorts() 中增加方向验证 | 1d | T3.2-02 | 无效连接被拒绝 |
| T3.2-05 | **Bundle 类型匹配检查 (L2)** | 1d | T3.2-02 | 类型不匹配被拒绝 |
| T3.2-06 | **数据宽度检查 (L3)** | 0.5d | T3.2-02 | 位宽不一致产生 WARNING |
| T3.2-07 | **端口别名系统** — GlobalPortAliases 结构体 + 解析 | 1.5d | T3.2-03 | `router.NORTH` → `router.0` |
...
| T3.2-17 | **更新架构文档** — 记录端口管理系统设计 | 0.5d | T3.2-16 | 文档评审通过 |
| T3.2-18 | **更新用户指南** — 端口配置指南 | 0.5d | T3.2-16 | 文档评审通过 |
...
**文档状态**: 📋 草稿（需与 Phase 3.1 代码实现对齐后开始实施）
