...
- 现有工具链重构（topology_generator.py, noc_builder.py）
...
#### G4: noc_builder.py 端口格式不兼容（P0）

**问题**: `python/noc_builder.py:42-48` 使用命名端口：

```python
connections.append({"src": f"{current}.E_out", "dst": f"{east}.W_in"})
```

C++ 端 `parsePortSpec` 解析 `.E_out` 得到 `spec="E_out"`，`std::isdigit('E')` 为 false，`src_idx` 保持默认值 0。

**影响**: 东向连接实际绑定到端口 0（NORTH），而非端口 1（EAST）。

---

#### G5: noc_mesh.py 不可运行（P1）

**问题**: `python/noc_mesh.py` 引用不存在的类：`VcRouter`、`TerminalNode`、`build_mesh_connections`、`connect_terminals`。
...
| G4 | noc_builder.py 端口格式 | P0 | 正确 | 无需修正 |
| G5 | noc_mesh.py 不可运行 | P1 | 正确 | 无需修正 |
